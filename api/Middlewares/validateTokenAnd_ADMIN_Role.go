...
func (m *Middlewares) ValidateTokenAnd_ADMIN(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if dataToken.Role != roles.ADMIN && dataToken.Role != roles.SUPER_ADMIN {
			response := responseDto.NewResponse(errorcodes.UNAUTHORIZED, "Don't have the necessary role", nil)
			return c.JSON(http.StatusUnauthorized, response)
		}
